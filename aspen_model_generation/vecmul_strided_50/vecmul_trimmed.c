...
#pragma aspen  control loads(0:from(a):traits(stride(50), pattern(stride)))
#pragma aspen  control loads(0:from(b):traits(stride(50), pattern(stride)))
#pragma aspen  control stores(0:from(c):traits(stride(50), initialized(0), pattern(stride)))
            c[i] = a[i] * b[i];
        }
  }
#ifdef _OPENACCM
