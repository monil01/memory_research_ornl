...
read = np.array(read) * 64
reads = np.array(reads) * 64
write = np.array(write) * 64
...
plt.plot(stride, read, marker='o', markersize=10,  color='black', linestyle='dashed', label="Read: no pref", linewidth=2)
...
plt.plot(stride, reads, marker='>', markersize=10,  color='green', linestyle='-.', label="Read: pref", linewidth=2)
...
plt.show()
fig.savefig('pref_sk.png', dpi=100)
