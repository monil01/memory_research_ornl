...
#pragma aspen  declare param(m_size:4096)
int m_size = 4096;
...
float (* a)[(4096+2)] = (float (* )[(4096+2)])malloc(((sizeof (float)*(m_size+2))*(m_size+2)));
#pragma aspen  declare data(b:traits(Matrix((2+m_size), (2+m_size))))
float (* b)[(4096+2)] = (float (* )[(4096+2)])malloc(((sizeof (float)*(m_size+2))*(m_size+2)));
...
free(a);
free(b);
a_CPU=((float (* )[(4096+2)])malloc(((sizeof (float)*(m_size+2))*(m_size+2))));
b_CPU=((float (* )[(4096+2)])malloc(((sizeof (float)*(m_size+2))*(m_size+2))));
for (i=0; i<(m_size+2); i ++ )
{
for (j=0; j<(m_size+2); j ++ )
{
