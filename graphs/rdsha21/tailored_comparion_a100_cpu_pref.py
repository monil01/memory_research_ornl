...
stride = ['1', '2', '4', '8', '16', '32', '64', '128', '256', '512', '1024', '2048', '4096']
...
read_p100=[800000800,1199420576,1199616416,1199429920,599708064,299862112,149854336,74863872,37356192,18602016,9222240,4533120,2186144]
write_p100=[399888448,400825440,401014272,400811072,201077184,101228320,51176864,26263168,13756288,7435680,4239136,2638624,1888768]


read_a100=[800010000,1190000000,1190000000,1190000000,996640000,498340000,248340000,123340000,60800000,29590000,14050000,6550000,3170000]
write_a100=[390160000,393490000,393170000,393140000,196570000,98290000,48320000,23320000,10200000,4300000,1320000,282110,31870]


reads=[13171615,18772132,18764571,18759543,16576687,11395672,2952856,1175004,588251,295965,148305,74884,37756]
#reads=[18765642,18772132,18764571,18759543,18764915,16576687,11395672,2952856,1175004,588251,295965,148305,74884,37756]

read_no_pref=[13171615,19005683,18970043,18940397,9468337,4724431,2363247,1182735,594024,296881,149252,75072,37848]
writes=[7151216,6608016,6587184,6584156,3296237,1644959,822515,411790,205808,103543,56143,29479,14986]
...
plt.plot(stride, read_a100, marker='s', markersize=7,  color='green', linestyle='dashed', label="Read - A100", linewidth=2)
#plt.plot(stride, write_a100, marker='>', markersize=10,  color='green', linestyle='dashed', label="Write - A100", linewidth=2)


plt.plot(stride, reads, marker='>', markersize=8,  color='red', linestyle='-.', label="Read: pref CPU", linewidth=2)
#plt.plot(stride, writes, marker='P', markersize=4,  color='black', linestyle='dashdot', label="WR-SK-Pf", linewidth=1)

plt.plot(stride, read_no_pref, marker='o', markersize=5,  color='black', linestyle='dashed', label="Read: no pref CPU", linewidth=2)
plt.plot(stride, writes, marker='<', markersize=5,  color='black', linestyle='solid', label="Write CPU", linewidth=1)
...
plt.show()
fig.savefig('tailored_comparison.png', dpi=100)
