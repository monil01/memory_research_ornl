 ...
 *-----------------------------------------------------------------------
*/
#pragma aspen  declare data(a:traits(Array(150000000, aspen_param_sizeof_float)))
static float a[(150000000+0)];
#pragma aspen  declare data(b:traits(Array(150000000, aspen_param_sizeof_float)))
static float b[(150000000+0)];
#pragma aspen  declare data(c:traits(Array(150000000, aspen_param_sizeof_float)))
static float c[(150000000+0)];
...
static double bytes[4] = {(2*sizeof (float))*150000000, (2*sizeof (float))*150000000, (3*sizeof (float))*150000000, (3*sizeof (float))*150000000};
...
printf("-------------------------------------------------------------\n");
printf("Array size = %llu (elements), Offset = %d (elements)\n", ((unsigned long long)150000000), 0);
printf("Memory per array = %.1f MiB (= %.1f GiB).\n", (BytesPerWord*((((double)150000000)/1024.0)/1024.0)), (BytesPerWord*(((((double)150000000)/1024.0)/1024.0)/1024.0)));
printf("Total memory required = %.1f MiB (= %.1f GiB).\n", ((3.0*BytesPerWord)*((((double)150000000)/1024.0)/1024.0)), ((3.0*BytesPerWord)*(((((double)150000000)/1024.0)/1024.0)/1024.0)));
...
aAvgErr=(aSumErr/((float)150000000));
bAvgErr=(bSumErr/((float)150000000));
cAvgErr=(cSumErr/((float)150000000));
if (sizeof (float)==4)
{
epsilon=1.0E-6;
}
...
ssize_t j;
#pragma omp parallel for
#pragma aspen  control label(block_tuned_STREAM_Triad3) loop(150000000) parallelism(150000000) flops(1:traits(integer))
for (j=0; j<150000000; j ++ )
{
#pragma aspen  control execute label(block_tuned_STREAM_Triad4) flops(2:traits(sp, simd)) loads((1*aspen_param_sizeof_float):from(b):traits(stride(1)), (1*aspen_param_sizeof_float):from(c):traits(stride(1))) stores((1*aspen_param_sizeof_float):to(a):traits(stride(1)))
a[j]=(b[j]+(scalar*c[j]));
}
return ;
}
