...
#pragma aspen  declare param(N:200000000)
int N = 200000000;
int M = N;
int P = N;
double my_timer()
...
{
#pragma aspen  control label(block_vectorMultiplication_openacc8) loop((floor(((-200+N)/200))+1)) flops(1:traits(integer))
for (i=0; i<N; i=(i+200))
{
#pragma aspen  control execute label(block_vectorMultiplication_openacc48) flops((1*(floor(((-200+N)/200))+1)):traits(sp, simd)) loads(((1*aspen_param_sizeof_float)*(floor(((-200+N)/200))+1)):from(a):traits(stride(200), pattern(stride)))
#pragma aspen  control label(block_vectorMultiplication_openacc48) loads(((1*aspen_param_sizeof_float)*(floor(((-200+N)/200))+1)):from(b):traits(stride(200), pattern(stride)))
#pragma aspen  control label(block_vectorMultiplication_openacc48) stores(((1*aspen_param_sizeof_float)*(floor(((-200+N)/200))+1)):from(c):traits(stride(200), pattern(stride)))
c[i]=(a[i]*b[i]);
}
}
#pragma aspen  control ignore
