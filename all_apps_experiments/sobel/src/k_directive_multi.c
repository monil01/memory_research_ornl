    ...
    for (int count = 0; count < ROWS*COLS; count++) {
      ...
      for (int i = 0; i < 3; ++i) {
        ...
        for (int j = 0; j < 3; ++j) {
          ...
          unsigned int pixel;
#pragma aspen declare param(aspen_param_if_prob1:1)
#pragma aspen control probability(aspen_param_if_prob1)
          if (count - (i * COLS) - j >= 0) {
...
#pragma aspen declare param(aspen_param_if_prob2:0.5)
#pragma aspen control probability(aspen_param_if_prob2)
      if (temp > threshold) {
        clamped = 0xffffff;
      } else {
        clamped = 0;
