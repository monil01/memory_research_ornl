...
void kernel5(uint64_t nsize, uint64_t ntrials, double *__restrict__ A,
            int *bytes_per_elem, int *mem_accesses_per_elem) {
    *bytes_per_elem = sizeof(*A);
    *mem_accesses_per_elem = 2;
...
void kernel4(uint64_t nsize, uint64_t ntrials, double *__restrict__ A,
            int *bytes_per_elem, int *mem_accesses_per_elem) {
    *bytes_per_elem = sizeof(*A);
    *mem_accesses_per_elem = 2;
...
void kernel3(uint64_t nsize, uint64_t ntrials, double *__restrict__ A,
            int *bytes_per_elem, int *mem_accesses_per_elem) {
    *bytes_per_elem = sizeof(*A);
    *mem_accesses_per_elem = 2;
...
void kernel2(uint64_t nsize, uint64_t ntrials, double *__restrict__ A,
            int *bytes_per_elem, int *mem_accesses_per_elem) {
    *bytes_per_elem = sizeof(*A);
    *mem_accesses_per_elem = 2;
...
void kernel_triad(uint64_t nsize, uint64_t ntrials, double *__restrict__ A,
            int *bytes_per_elem, int *mem_accesses_per_elem) {
    *bytes_per_elem = sizeof(*A);
    *mem_accesses_per_elem = 2;
...
void kernel1(uint64_t nsize, uint64_t ntrials, double *__restrict__ A,
            int *bytes_per_elem, int *mem_accesses_per_elem) {
    *bytes_per_elem = sizeof(*A);
    *mem_accesses_per_elem = 2;
...
int main(int argc, char *argv[]) {
    ...
    if (*p_gpu != 20 && *p_gpu != 1) *p_gpu = -5;                           //*p = 0;
...
for (int ar = 2; ar <= argc; ar++){
	...
	/*	
	if (strcmp(argv[ar-1], "1") == 0) {fun_ptr = &kernel1;  n = 30833802;}
	if (strcmp(argv[ar-1], "2") == 0) {fun_ptr = &kernel2;  n = 5138967;}
	if (strcmp(argv[ar-1], "3") == 0) {fun_ptr = &kernel3;  n = 2569483;}
	if (strcmp(argv[ar-1], "4") == 0) {fun_ptr = &kernel4; n =  1541690;}
	if (strcmp(argv[ar-1], "5") == 0) { fun_ptr = &kernel5;  n = 642371;} */

	if (strcmp(argv[ar-1], "1") == 0) {fun_ptr = &kernel1; }
	if (strcmp(argv[ar-1], "2") == 0) {fun_ptr = &kernel2; }
	if (strcmp(argv[ar-1], "3") == 0) {fun_ptr = &kernel3; }
	if (strcmp(argv[ar-1], "4") == 0) {fun_ptr = &kernel4; }
	if (strcmp(argv[ar-1], "5") == 0) { fun_ptr = &kernel5;} 
	if (strcmp(argv[ar-1], "6") == 0) { fun_ptr = &kernel_triad;} 
	    ...
	    seconds = 0;
            ...
            for (t = lim - 35; t <= lim; t = t + 1) { // working set - ntrials
                ...
                if (t >= (lim - 19)){
                    *p_cpu = 10;
   			while (id == 0 && 1){
				if (id == 0 && (*p_gpu == 10 || *p_gpu == 1 || *p_gpu == 33 || *p_gpu ==-5)) break;
    				fprintf (stderr,"p_cpu=%d  p_gpu=%d is allocated in shared memory.\n\n", *p_cpu, *p_gpu);
//#pragma omp barrier
                		//(*fun_ptr)(1000, t, &buf[nid], &bytes_per_elem, &mem_accesses_per_elem);
//#pragma omp barrier
		        }
...
#pragma omp barrier
                ...
                // C-code
                (*fun_ptr)(n, t, &buf[nid], &bytes_per_elem, &mem_accesses_per_elem);
